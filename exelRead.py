...
for x in range(4, row_count+1):
    ...
    empd.dayoffcount=sheet['M'+str(x)].value
    empd.dayoffamount=pattern.format(sheet['N'+str(x)].value)
    empd.attendenceAllowence=pattern.format(sheet['O'+str(x)].value)
    empd.attendenceincentive=pattern.format(sheet['P'+str(x)].value)
    empd.epf8amount=pattern.format(sheet['Q'+str(x)].value)
    empd.totalearning=pattern.format(sheet['R'+str(x)].value)
    empd.epf12amount=pattern.format(sheet['S'+str(x)].value)
    empd.etf3=pattern.format(sheet['T'+str(x)].value)
    empd.grosssalary=pattern.format(sheet['U'+str(x)].value)
